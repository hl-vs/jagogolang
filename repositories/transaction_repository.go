...
// gpt5.2
func (repo *TransactionRepository) CreateTransactionV3(items []models.CheckoutItem) (*models.Transaction, error) {
	if len(items) == 0 {
...
func (repo *TransactionRepository) CreateTransactionV2(items []models.CheckoutItem) (*models.Transaction, error) {
	...
	for _, item := range items {
		...
		if item.Quantity > product.stock {
			return nil, fmt.Errorf("Stock %s unavailable", item.Quantity)
		}
